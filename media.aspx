<%@ Page Language="VB" AutoEventWireup="false" CodeFile="media.aspx.vb" Inherits="prizes" MasterPageFile="~/MasterPage.master" %>


<asp:Content runat="server" ContentPlaceHolderID="ContentArea">

    <h1>Наши игры в прессе</h1>
    <p>
        Читайте отчеты об играх в беслатной газете <a href="http://adandlife.com" target="_blank">"Реклама и Жизнь"</a> - страница 18.
    </p>

    <section id="game21">
        <h1>Очерк о XXI игре (21 октября 2017)</h1>
        <p>
            Опубликован в 43 номере газеты "Реклама и Жизнь" за 2017 год. Нажмите на изображение, чтобы открыть виртуальную газету.
        </p>
        <a target="_blank" href="https://www.docdroid.net/UQtvtXF/riz43.pdf#page=18">
            <img src="/Img/media/game21.jpg" />
        </a>
    </section>

    <section id="game20">
        <h1>Очерк о XX игре (5 августа 2017)</h1>
        <p>
            Опубликован в 32 номере газеты "Реклама и Жизнь" за 2017 год. Нажмите на изображение, чтобы открыть виртуальную газету.
        </p>
        <a target="_blank" href="https://www.docdroid.net/UE5Nzbv/32compressed.pdf#page=18">
            <img src="/Img/media/game20.jpg" />
        </a>
    </section>

    <section id="game19">
        <h1>Очерк о XIX игре (1 июля 2017)</h1>
        <p>
            Опубликован в 27 номере газеты "Реклама и Жизнь" за 2017 год. Нажмите на изображение, чтобы открыть виртуальную газету.
        </p>
        <a target="_blank" href="https://www.docdroid.net/I5BCmQB/riz27.pdf#page=18">
            <img src="/Img/media/game19.jpg" />
        </a>
    </section>

    <section>
        <h1>Очерк о майской XVIII игре</h1>
        <p>
            Опубликован в 20 номере газеты "Реклама и Жизнь" за 2017 год. Нажмите на изображение, чтобы открыть виртуальную газету.
        </p>
        <a target="_blank" href="https://www.docdroid.net/aDApt37/20compressed.pdf.html#page=18">
            <img src="/Img/media/game18.jpg" />
        </a>
    </section>

    <section>
        <h1>Очерк о первоапрельской XVII игре</h1>
        <p>
            Опубликован в 14 номере газеты "Реклама и Жизнь" за 2017 год. Нажмите на изображение, чтобы открыть виртуальную газету.
        </p>
        <a target="_blank" href="https://www.docdroid.net/qVEo4s8/riz14.pdf.html#page=18">
            <img src="/Img/media/game17.jpg" />
        </a>
    </section>
    
    <section>
        <h1>Очерк о февральской XVI игре</h1>
        <p>
            Опубликован в 9 номере газеты "Реклама и Жизнь" за 2017 год. Нажмите на изображение, чтобы открыть виртуальную газету.
        </p>
        <a target="_blank" href="https://www.docdroid.net/m6p4fDV/riz9.pdf.html#page=18">
            <img src="/Img/media/game16.jpg" width="715" />
        </a>
    </section>
    
    

</asp:Content>
